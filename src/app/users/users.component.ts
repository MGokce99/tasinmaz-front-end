...
export class UsersComponent implements OnInit {
  ...
  navigateToUpdateUser() {
    if (this.selectedUsers.length === 1) {
      ...
      this.alertifyService.error(
        "Lütfen güncellemek için sadece bir adet kullanıcı seçin"
      );
    }
  }
    ...
    if (this.searchText.trim() === "") {
      ...
      const filteredUsers = this.users.filter((user) => {
        return (
          ...
          (user.adres &&
            user.adres.toLowerCase().includes(this.searchText.toLowerCase()))
        );
      });
  ...
  generateReport() {
    if (this.selectedUsers.length !== 1) {
      this.alertifyService.error(
        "Lütfen raporlamak için en az bir kullanıcı seçin."
      );
      return;
    }
    ...
    this.alertifyService.success(
      "Kullanıcı bilgileri raporları başarıyla iletildi"
    );
  }
      ...
      this.alertifyService.confirm(
            ...
            .then((results) => {
              results.forEach((result, index) => {
                if (result) {
                  ...
                  this.alertifyService.success(
                    `Kullanıcı ID ${this.selectedUsers[index].userId} başarıyla silindi.`
                  );
                  // Kullanıcıları yerel listeden kaldır
                  this.users = this.users.filter(
                    (item) => item.userId !== this.selectedUsers[index].userId
                  );
                  this.updatePagedUsers();
                } else {
                  // Hata mesajı göster
                  this.alertifyService.error(
                    `Kullanıcı ID ${this.selectedUsers[index].userId} silinemedi.`
                  );
                }
              });
        ...
        () => {
          ...
          this.alertifyService.warning(
            "Kullanıcı Silme İşlemini İptal Ettiniz..."
          );
        }
      );
    } else {
      this.alertifyService.error(
        "Lütfen silmek için en az bir kullanıcı seçin."
      );
    }
  }
